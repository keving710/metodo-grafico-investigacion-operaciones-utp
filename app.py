...
def index():
    ...
    if request.method == "POST":
        ...
        n = int(request.form["num_restricciones"])

        for i in range(n):
            a = float(request.form[f"a{i}"])
            b = float(request.form[f"b{i}"])
            signo = request.form[f"signo{i}"]
            c = float(request.form[f"c{i}"])
        ...
        grafico = graficar(restricciones, vertices, mejor_punto)
